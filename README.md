...
`npm run dev` 后打开 **http://127.0.0.1:5178/workbench.html**，一个页面管全部：

- **剪辑方案**：每个 `episodes/<工程>` 就是一个方案，登记进总库管理版本与状态（稳定 ID / 版本 / 生产状态 / 共享边界）
- **动画与动效**：内置两套开箱可用的动画库，点名称即活预览——
  - **语义动画库**（16 种原子语法：遮罩揭示 / 路径跟随 / 形态变换 / 分裂合并 / 级联序列……按口播语义选动画，不按"好看"选）
  - **动画丰富度扩展**（漫画分格 / 速度隧道 / 磁力聚合 / 压力堆叠 / 分叉赛跑 / 波纹反馈 6 种叙事语法）
- **引擎与参数**：编辑器组件引擎、初剪算法等系统资产的版本索引
- **真片编辑**：AI 视频编辑器就嵌在第四个 tab 里，不用切页面
- **＋登记资产**：自己做的动效 / 剪辑方案填个表就进总库，和内置资产一样被管理和预览
